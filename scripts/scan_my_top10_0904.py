# -*- coding: utf-8 -*-
"""READ-ONLY: MY per-creative 60d CPL + CPA + sales record, for the owner's
...
Aggregates by normalised creative name across every instance:
  ...
  new-account sales in 60d (sheet rows whose UTM campaign starts 'stockbloom'),
...
def main() -> None:
    ...
    print(f"MY per-creative 60d CPL+CPA+成交 — {SINCE} → {TODAY}（read-only）\n")
